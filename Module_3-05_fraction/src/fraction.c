...
{
    unsigned int comDenom = a->denominator * b->denominator;
    unsigned int fracA = (comDenom / a->denominator) * a->numerator;
    unsigned int fracB = (comDenom / b->denominator) * b->numerator;
...
Fraction* addFraction(const Fraction *a, const Fraction *b)
{
    ...
    unsigned int fracA = (comDenom / a->denominator) * a->numerator;
    unsigned int fracB = (comDenom / b->denominator) * b->numerator;
    unsigned int comNum = fracA + fracB;
    return setFraction(comNum, comDenom);
}
...
{
    int num = gcd(val->numerator, val->denominator);
    val->numerator /= num;
    val->denominator /= num;
}
